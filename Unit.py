...
class Object:
    ...
    @classmethod
    def create_object_from_infos(cls, w: int, h: int, infos: dict):
...
class Unit(Object):
    ...
    def __init__(
        ...
        self.default_fuel = object_type.unit_type[name][object_type.FUEL]
        if fuel == None:
            self.fuel = self.default_fuel
        else:
            self.fuel = fuel

        self.damage = object_type.unit_type[name][object_type.DAMAGE]

        self.range = object_type.unit_type[name][object_type.RANGE]

        self.view_range = object_type.unit_type[name][object_type.VIEW_RANGE]

        self.movement_point = object_type.unit_type[name][object_type.MOVEMENT_POINT]
    ...
    def create_object_from_infos(cls, w: int, h: int, infos: dict):
        ...
        type, name, team, pv = super().create_object_from_infos(w, h, infos)
...
class Usine(Object):
    def __init__(self, w, h, name, team, pv_of_unit=None, ammo=None, fuel=None):
        super().__init__(w, h, name, team, pv_of_unit, ammo, fuel)
