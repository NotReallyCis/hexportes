...
class UI:
    ...
    def __init__(self, rect: pg.Rect, surface: pg.Surface | pg.Color = (0, 0, 0, 0)):
        self.rect = rect
        self.elements: list[UI.Element] = []
        ...
        UI.all_menus.append(self)

    def draw(self):
        camera(self.surface, self.rect, -55, True)
    ...
    class Element:
        def __init__(
            ...
            menu: "UI",
            rect: pg.Rect = None,
            background: pg.Surface | pg.Color = None,
        ):
            self.menu = menu
            self.menu.elements.append(self)
            if rect == None:
                self.rect = self.menu.rect.copy()
            else:
                self.rect = rect.move(
                    self.menu.rect.x, self.menu.rect.y
                )  # make it relative to the menu

                self.rect = self.rect.clip(self.menu.rect)

            if not self.rect.colliderect(self.menu.rect):
                raise ValueError(
                    f"the rect:{self.rect} isn't inside the rect ({self.menu.rect}) of the menu"
                )

            if isinstance(background, pg.Surface):
                self.surface = pg.transform.scale(background.copy(), self.rect.size)
    ...
    class Button_box(Element):
        def __init__(
            ...
            function_args: tuple = (),
            right_click_function: "function" = None,
            right_click_function_args: tuple = (),
        ):
            super().__init__(menu, rect, background)
            self.button = Button(
                self.surface,
                function,
                self.rect,
        ...
        def update(
            ...
            args_for_function: tuple = None,
        ):

            if surface != None:
                self.surface = surface
                self.button.set_surface(surface)
            if rect != None:
                self.rect = rect
                self.button.rect = rect
            if function != None:
                self.button.lmb_function = function
            if args_for_function != None:
                self.button.lmb_args = args_for_function

        def show(self):
            super().show()
            self.button.is_visible = True
    ...
    class Text_box(Element):
        def __init__(
            ...
            text: str,
            rect: pg.Rect = None,
            rect_for_text: pg.Rect = None,
            background: pg.Surface | pg.Color = None,
            font: pg.font.Font = draw.default_font,
            text_color: pg.Color = "black",
        ):
            """Create a text surface

            Args:
                menu (UI): the menu which it's part of
                text (str): _description_
                rect (pg.Rect, optional): The rect of the Surface globally . Defaults to None.
                rect_for_text (pg.Rect, optional): The sub rect which the text will be shown in. Defaults to None.
                background (pg.Surface | pg.Color, optional): The surface/color. Defaults to None.
                font (pg.font.Font, optional): _description_. Defaults to draw.default_font.
                text_color (pg.Color, optional): _description_. Defaults to "black".
            """
            ...
            if rect_for_text == None:

                self.rect_for_text = self.rect.copy()
            else:
                self.rect_for_text = rect_for_text
                self.rect_for_text = rect_for_text.move(
                    self.menu.rect.x, self.menu.rect.y
                )  # make it relative to the menu

                self.rect_for_text = self.rect_for_text.clip(self.rect)

        def set_new_text(self, text: str):
            self.text = str(text)

        def draw(self):
            camera(self.surface, self.rect, -51, True)

            text_surface = draw.text(
                self.text, self.text_color, self.font, self.rect_for_text
            )
            camera(text_surface, self.rect_for_text, -50, True)
